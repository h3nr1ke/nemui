...
export function registerCommands(context: vscode.ExtensionContext, collectionsProvider: CollectionsTreeProvider) {
    ...
    // Delete Collection
    context.subscriptions.push(
        vscode.commands.registerCommand('nemui.deleteCollection', async (collection: ApiCollection) => {
            ...
            if (confirm === 'Delete') {
                collectionsProvider.deleteCollection(collection.id);
    ...
    // Delete Request
    context.subscriptions.push(
        vscode.commands.registerCommand('nemui.deleteRequest', async (request: ApiRequest) => {
            if (!request || !request.id) {
                vscode.window.showWarningMessage('Select a request first');
                return;
            }
            ...
            if (confirm === 'Delete') {
                collectionsProvider.deleteRequest(request.id);
                vscode.window.showInformationMessage(`Request "${request.name}" deleted!`);
            }
        })
    );
}
